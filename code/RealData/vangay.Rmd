...
for(i in 1:length(P.list)){
  ...
  dist_matrix = dist(t(P_sub))
  df.Y = as.data.frame(Y)
  Re = adonis2(dist_matrix~Y, data = df.Y,permutations = 9999)
...
for(i in 1:length(P.list)){
  ...
  P_sub = P2[,c(ind1,ind2)]
  if(is.null(p)){
    p = mds.plot(log(P_sub+1),Y,method.list[i],pvalue[i])
  }else{
    p = p + mds.plot(log(P_sub+1),Y,method.list[i],pvalue[i])
  }
}
p+ plot_layout(ncol = 7,guides = "collect")& theme(legend.position = "bottom") 
```
...
#write.csv(res,"../Realdata/KarenThai_bmi.csv")
```
